...
def get_source_loader(dataset_name: str, adaptation: str, preprocess: Union[transforms.Compose, None],
                      ...
                      num_samples: int = -1, percentage: float = 1.0, workers: int = 4, target_classes=None):
    """
    ...
    # 减少样本数量逻辑前添加保留特定类别样本的逻辑
    if target_classes is not None:
        assert isinstance(target_classes, list) and len(target_classes) > 0, "target_classes must be a non-empty list"

        # 初始化筛选后的样本和标签列表
        filtered_data = []
        filtered_targets = []
        # 对于 CIFAR10 或 CIFAR100 等数据集的处理
        if src_dataset_name in ["cifar10", "cifar100"]:
            class_samples = {target: [] for target in target_classes}

            for i, label in enumerate(source_dataset.targets):
            ...
            if num_samples > 0:
                ...
                total_samples = int(np.ceil(len(source_dataset.data) * percentage))
                samples_per_class = total_samples // len(target_classes)
            all_selected_indices = []
            for target in target_classes:
                selected_indices = random.sample(class_samples[target], min(samples_per_class, len(class_samples[target])))
                all_selected_indices.extend(selected_indices)
                for idx in selected_indices:
                    filtered_data.append(source_dataset.data[idx])
                    filtered_targets.append(source_dataset.targets[idx])
            print(all_selected_indices)
            source_dataset.data = np.array(filtered_data)
            source_dataset.targets = filtered_targets

    elif percentage < 1.0 or num_samples >= 0:    # reduce the number of source samples
...
def get_test_loader_subset(setting: str, adaptation: str, dataset_name: str, preprocess: Union[transforms.Compose, None],
                    data_root_dir: str, domain_name: str, domain_names_all: list, severity: int, num_examples: int,
                    rng_seed: int, delta_dirichlet: float = 0., batch_size: int = 128, shuffle: bool = False,
                    workers: int = 4, target_classes=None, num_samples: int = -1):
    """
    Create the test data loader
    Input:
        setting: Name of the considered setting
    ...
    try:
        # 减少样本数量逻辑前添加保留特定类别样本的逻辑
        if target_classes is not None:
            assert isinstance(target_classes, list) and len(
                target_classes) > 0, "target_classes must be a non-empty list"
            ...
            if dataset_name in ["cifar10_c", "cifar100_c"]:
                ...
                for i, single_sample in enumerate(test_dataset.samples):
                    label = single_sample[1]
                ...
                for target in target_classes:
                    selected_indices = random.sample(class_samples[target],
                                                     min(samples_per_class, len(class_samples[target])))
                    all_selected_indices.extend(selected_indices)
                    for idx in selected_indices:
                        filtered_data.append(test_dataset.samples[idx])

                test_dataset.samples = np.array(filtered_data)
                print(domain_name, all_selected_indices)
    except AttributeError:
        logger.warning("Attribute 'samples' is missing. Continuing without shuffling, sorting or subsampling the files...")
